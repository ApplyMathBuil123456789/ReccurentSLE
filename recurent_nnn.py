...
Original file is located at
    https://colab.research.google.com/drive/1RNPJ2kafcEY_T37sgwes73aNyMXCpSci
"""
...
class SpectralReconstructor(nn.Module):
    ...
    def _get_phi_matrix(self, j_vals: torch.Tensor, t_vals: torch.Tensor) -> torch.Tensor:
        """
        j_vals: 1, 2, ..., l  (l ta bazis)
        qaytaradi: (l, M) ortonormal bazis matritsasi
          j=1 -> phi_1 = 1 (DC)
        ...
        # Qolgan bazis funksiyalar (j=2..l)
        j_rem = j_vals[1:]                     # (l-1,)
        k = torch.floor(j_rem / 2)             # to'lqin raqami
        angles = 2 * math.pi * torch.tensordot(k, t_vals, dims=0)  # (l-1, M)

        # juft j -> sin, toq j -> cos (j indeksi 2 dan boshlangani uchun)
...
# Modelni saqlash uchun fayl nomi
model_save_path = 'recurrent_sle_model_005.pth'

# Modelning state_dict (o'qitilgan parametrlar) qismini saqlash
# Bu eng tavsiya etilgan usul, chunki u modelning arxitekturasini kodda saqlash imkonini beradi.
torch.save(model.state_dict(), model_save_path)

print(f"Model muvaffaqiyatli saqlandi: {model_save_path}")

# =============================================================================
# Saqlangan modelni boshqa muhitda qanday qo'llash
# =============================================================================

print("\n--- Saqlangan modelni boshqa muhitda yuklash va ishlatishga doir ko'rsatma ---")
print("Boshqa muhitda modelni yuklash va ishlatish uchun quyidagi qadamlar zarur:")
print("1. Ushbu notebookdagi barcha model sinflari (ModelConfig, ParameterGrid, SpectralReconstructor, RecurrentSLE, va SignalDatasetGenerator kabi) aniqlanganligiga ishonch hosil qiling.")
print("   Buning uchun, 'AdtHgfGm6fBu' nomli asosiy kod katakchasini yangi muhitingizda ishlating.")
print("2. Saqlangan model fayli ('recurrent_sle_model.pth') yangi muhitda mavjud bo'lishi kerak.")

# Misol kod: Yuklash jarayoni (ushbu katakchada ishlatilmaydi, faqat ko'rsatma)
print("\nQuyidagi kod parchasi saqlangan modelni qanday yuklash va ishlatishni ko'rsatadi:")

loading_example_code = """
# 1. Zarur bo'lgan kutubxonalarni import qilish
import torch
import numpy as np

# 2. Model konfiguratsiyasi va boshqa yordamchi sinflarni aniqlash
#    (Yangi muhitda asosiy model kodini (AdtHgfGm6fBu) ishga tushirish talab qilinadi)
#    model = RecurrentSLE(...) yaratish uchun config va param_grid kerak.

# Avvalgi o'qitishda ishlatilgan konfiguratsiyani qayta yaratish
...
# Modelning yangi nusxasini yaratish (parametrlarsiz)
...
# Modelni yuklanadigan qurilmaga ko'chirish (CPU yoki GPU)
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
loaded_model.to(device)

# 3. Saqlangan state_dict ni yuklash
model_save_path = 'recurrent_sle_model.pth' # Fayl yo'li
loaded_model.load_state_dict(torch.load(model_save_path, map_location=device))

# 4. Modelni baholash rejimiga o'tkazish
loaded_model.eval()

print(f"Model '{model_save_path}' muvaffaqiyatli yuklandi.")

# 5. Yuklangan modeldan foydalanishga misol:
# Yangi shovqinli signalni yaratish (misol uchun)
loaded_generator = SignalDatasetGenerator(loaded_config)
Y_new, S_clean_new, _ = loaded_generator.generate_single_type(
    n_samples=1, signal_type=0, noise_eps=0.1 # Sinusoidal signal, 0.1 shovqin
)

# Signalni modelga moslashtirish
noisy_signal_to_denoise = Y_new[0].astype(np.float32)

# Shovqinlardan tozalash
with torch.no_grad():
...
print("Yangi signal muvaffaqiyatli shovqinlardan tozalandi.")
"""

print(f"""```python\n{loading_example_code}\n```""")

print("Yuqoridagi ko'rsatmalar va kod parchasi yordamida o'qitilgan modelni istalgan muhitda qayta ishlatishingiz mumkin.")
...
# ------------------------------
# 1. HeaviSine signalini yaratish
# ------------------------------
def heavisine(t):
    """
    HeaviSine signali - Donoho (1994) tomonidan taklif qilingan test signali
    f(t) = 4 * sin(4 * pi * t) - sgn(t - 0.3) - sgn(0.72 - t)
    """
    # 4*sin(4*pi*t) qismi
    sin_part = 4 * np.sin(4 * np.pi * t)

    # sgn(t - 0.3) qismi: t < 0.3 bo'lsa -1, t >= 0.3 bo'lsa 1
    sgn1 = np.where(t < 0.3, -1, 1)

    # sgn(0.72 - t) qismi: t < 0.72 bo'lsa 1, t >= 0.72 bo'lsa -1
...
# ------------------------------
# 2. Parametrlarni belgilash
# ------------------------------
np.random.seed(42)  # Natijalarni takrorlash uchun

# Vaqt diskretizatsiyasi
n = 2048  # signal uzunligi (maqoladagi kabi yetarlicha katta)
t = np.linspace(0, 1, n, endpoint=False)

# Asl shovqinsiz signal
signall=heavisine(t)
scale = np.max(np.abs(signall))
signal_clean = signall/scale

# Shovqin parametrlari
sigma = 0.2  # standart og'ish (maqoladagi kabi)

print(scale)
noise = np.random.normal(loc=0, scale=sigma, size=n)/scale

# Shovqinli signal (additive noise)
...
# ------------------------------
print("=" * 50)
print("SHOVQIN STATISTIKASI")
print("=" * 50)
print(f"Shovqin o'rtacha qiymati (mean): {np.mean(noise):.6f}")
print(f"Shovqin standart og'ishi (std):  {np.std(noise):.6f}")
print(f"Kutilgan sigma:                  {sigma}")
print(f"Shovqin variatsiyasi (var):      {np.var(noise):.6f}")
print(f"Kutilgan sigma^2:                {sigma**2}")

# SNR (Signal-to-Noise Ratio) ni hisoblash
signal_power = np.mean(signal_clean ** 2)
noise_power = np.mean(noise ** 2)
snr = 10 * np.log10(signal_power / noise_power)
print(f"\nSignal-Shovqin nisbati (SNR): {snr:.2f} dB")

# ------------------------------
# 4. Vizualizatsiya (maqoladagi 2-rasmga o'xshash)
# ------------------------------
fig, axes = plt.subplots(2, 1, figsize=(14, 8))

# Yuqori panel: Shovqinsiz signal
axes[0].plot(t, signal_clean, 'b-', linewidth=1, label='Shovqinsiz signal (Noise-free)')
axes[0].set_xlim(0, 1)
axes[0].set_ylim(-6, 6)
axes[0].set_ylabel('Amplituda', fontsize=12)
axes[0].set_title('HeaviSine signali - shovqinsiz (Noise-free)', fontsize=14)
axes[0].grid(True, alpha=0.3)
axes[0].legend(loc='upper right')

# Pastki panel: Shovqinli signal
axes[1].plot(t, signal_noisy, 'r-', linewidth=0.8, alpha=0.7, label=f'Shovqinli signal ($\\sigma$ = {sigma})')
# Asl signalni ham ustiga chizish (taqqoslash uchun)
axes[1].plot(t, signal_clean, 'b-', linewidth=1, alpha=0.5, label='Asl signal')
axes[1].set_xlim(0, 1)
axes[1].set_ylim(-6, 6)
axes[1].set_xlabel('Vaqt (t)', fontsize=12)
axes[1].set_ylabel('Amplituda', fontsize=12)
axes[1].set_title('HeaviSine signali - qo\'shilgan Gaussian shovqin bilan', fontsize=14)
axes[1].grid(True, alpha=0.3)
...
# ------------------------------
# 5. Kichik bir qismni yaqindan ko'rish (shovqin effektini aniqroq ko'rish uchun)
# ------------------------------
fig, ax = plt.subplots(figsize=(14, 5))

# Vaqtning 0.2 dan 0.4 gacha bo'lgan qismini ko'rsatamiz
mask = (t >= 0.2) & (t <= 0.4)
t_zoom = t[mask]
clean_zoom = signal_clean[mask]
noisy_zoom = signal_noisy[mask]

ax.plot(t_zoom, clean_zoom, 'b-', linewidth=2, label='Asl signal (shovqinsiz)')
ax.plot(t_zoom, noisy_zoom, 'r-', linewidth=1, alpha=0.8, label='Shovqinli signal')
ax.fill_between(t_zoom, clean_zoom, noisy_zoom, alpha=0.3, color='gray', label='Shovqin (farq)')
ax.set_xlabel('Vaqt (t)', fontsize=12)
ax.set_ylabel('Amplituda', fontsize=12)
ax.set_title(f'HeaviSine signali - yaqindan ko\'rinish (shovqin std = {sigma})', fontsize=14)
...
# ------------------------------
# 6. Shovqin taqsimotini tekshirish (Gaussian ekanligini tasdiqlash)
# ------------------------------
fig, ax = plt.subplots(1, 2, figsize=(12, 4))

# Histogram
ax[0].hist(noise, bins=50, density=True, alpha=0.7, color='gray', edgecolor='black')
# Gaussian egri chizig'i
x = np.linspace(-0.8, 0.8, 100)
gaussian = stats.norm.pdf(x, 0, sigma)
ax[0].plot(x, gaussian, 'r-', linewidth=2, label=f'N(0, {sigma}²)')
ax[0].set_xlabel('Shovqin qiymati', fontsize=12)
ax[0].set_ylabel('Zichlik', fontsize=12)
ax[0].set_title('Shovqin taqsimoti (histogramma)', fontsize=12)
ax[0].legend()
ax[0].grid(True, alpha=0.3)

# Q-Q plot (normal probability plot)
stats.probplot(noise, dist="norm", plot=ax[1])
ax[1].set_title('Q-Q plot (normalitet tekshiruvi)', fontsize=12)
...
# ------------------------------
# 7. Turli sigma qiymatlari uchun taqqoslash
# ------------------------------
...
for idx, s in enumerate(sigmas):
    ...
    axes[idx].plot(t, signal_clean, 'b-', linewidth=1, alpha=0.5, label='Asl')
    axes[idx].plot(t, signal_temp, 'r-', linewidth=0.8, alpha=0.7, label=f'Shovqinli ($\\sigma$={s})')
...
print("\n" + "=" * 50)
print("TUGADI: Barcha grafiklar saqlandi")
print("=" * 50)

import numpy as np
import matplotlib.pyplot as plt
from scipy.stats import pearsonr # NCC hisoblash uchun

# Eslatma: Agar trainer va model ob'ektlari qayta ishga tushirilmagan bo'lsa,
# iltimos, 'main()' funksiyasini chaqiradigan katakchani qayta ishga tushiring.
# Modelning ichki 'M' konfiguratsiyasi 2048 bo'lishi kerak.

# 1. Shovqinli signalni model orqali tozalash
print("Shovqinli signalni model orqali tozalash...")
# signal_noisy ni float32 turiga o'tkazish muhim
denoised_signal = trainer.predict_single(signal_noisy.astype(np.float32))
print("Shovqinni tozalash yakunlandi.")

# 2. MSE (Mean Squared Error) hisoblash
# Taqqoslash uchun toza signalni ham float32 ga o'tkazish
mse = np.mean((signal_clean.astype(np.float32) - denoised_signal)**2)
print(f"\nMean Squared Error (MSE): {mse:.6f}")

# 3. SNR (Signal-to-Noise Ratio) hisoblash
...
# SNR hisoblashda toza va shovqinli signallarni float32 ga o'tkazish
...
print(f"Shovqinli signalning SNR: {snr_noisy:.2f} dB")
print(f"Tozalangan signalning SNR: {snr_denoised:.2f} dB")

# 4. NCC (Normalized Cross-Correlation) hisoblash
def calculate_ncc(signal1, signal2):
    # Pearson korrelatsiya koeffitsiyenti NCC ning keng tarqalgan shaklidir
...
print(f"Shovqinli signalning NCC: {ncc_noisy:.4f}")
print(f"Tozalangan signalning NCC: {ncc_denoised:.4f}")

# 5. Natijalarni bitta plotda taqdim etish
print("Natijalarni grafikda ko'rsatish...")
...
plt.rcParams.update({
    ...
    'xtick.labelsize': 18,
    'ytick.labelsize': 18,
    'figure.titlesize': 20 # Umumiy sarlavha uchun
})
...
# plt.title('Asl, Shovqinli va Qayta Ishlangan Signalni Taqqoslash')
plt.xlabel('Time')
plt.ylabel('Amplitude')
plt.legend()
...
plt.tight_layout()
plt.show()
print("Grafik muvaffaqiyatli ko'rsatildi.")

import numpy as np

def compute_snr(original, denoised):
    """
    Signal-to-Noise Ratio (SNR) ni hisoblaydi.
    Formula: SNR = 10 * log10( sum(x^2) / sum((y - x)^2) )

    Parameters:
    original : ndarray - asl shovqinsiz signal (x)
    denoised : ndarray - tozalangan signal (y)

    Returns:
    snr : float - desibeldagi SNR qiymati
    """
...
def compute_mse(original, denoised):
    """
    Mean Squared Error (MSE) ni hisoblaydi.
    ...
    Returns:
    mse : float - o'rtacha kvadratik xato
    """
    original = np.asarray(original)
    denoised = np.asarray(denoised)
...
def compute_ncc(original, denoised):
    """
    Normalized Cross-Correlation (NCC) ni hisoblaydi.
    ...
    Returns:
    ncc : float - korrelyatsiya koeffitsienti (0 dan 1 gacha, 1 eng yaxshi)
    """
    original = np.asarray(original)
    denoised = np.asarray(denoised)
